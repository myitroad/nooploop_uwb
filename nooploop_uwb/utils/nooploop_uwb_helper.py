...
def hex_to_decimal(hex_array):
    ...
    # Unpack the bytes in little-endian format (assuming 3 bytes) with signed interpretation
    value = int.from_bytes(hex_array, byteorder='little', signed=True)

    return value

# if __name__ == '__main__':
#     # Create AOA Instance with config.json
#     # rst = __hex2dec_ltendian_3bytes(0x00,0x0b,0x4f)
#     # rst = __hex_to_decimal('4f0b00');
#     rst = __hex_to_decimal('f9 fe ff');
#     # rst = __hex2dec_ltendian_3bytes(0x00,0x0b,0x4f)
#     print(rst)

def __parse_node_quantity(buffer_list: list, head_pos):
    return buffer_list[head_pos + 118]
...
def __verify_length(buffer_list: list, head_pos, frame_length):
    ...
    if frame_length != base_length + 13 * node_quantity:
...
def find_frameheader(buffer_list: list):
    ...
    while True:
        ...
        if buffer_list[header_index + 1] == 0x04:
            return header_index
        else:
