...
class AOA(object):
    ...
    def __init__(self, config_path=None, port='/dev/ttyUSB1', baudrate=921600):
        """
        Input:
            config_path: str
        """
    ...
    def get_data(self, timeout=None):
        ...
        self.valid_node_quantity = frame_data[118]
        # self.voltage = (frame_data[18] | frame_data[19] << 8) / 1000
        self.pos_x = hex_to_decimal(frame_data[13:16]) / 1000.0
        self.pos_y = hex_to_decimal(frame_data[16:19]) / 1000.0
        self.pos_z = hex_to_decimal(frame_data[19:22]) / 1000.0
        self.node = []

        for i in range(self.valid_node_quantity):
            self.node.append(AOA_Anchor(frame_data[119 + 13 * i: 132 + 13 * i]).get_data())
...
class AOA_Anchor(object):
    ...
    def __str__(self):
        s = "role: {:>6}\nid: {:>6}\ndistance: {:>6}\nangle: {:>6}\nfp_rssi: {:>6}\nrx_rssi: {:>6}".format(
            self.get_role(), self.get_id(), self.get_dist(), self.get_angle(), self.get_fp_rssi(), self.get_rx_rssi())
        return s
